...
class AE(AutoEncoder):
    ...
    def predict(self, x, **keras_params):
        ...
        preds = pd.DataFrame(index=x.index,columns=x.columns,
                                            data=super(AE, self).predict(x.values,keras_params))
        errors = (x-preds)**2
        return preds, errors
    ...
    def detect(self, x, theta, window = 1, average=False, sys_theta = 0, **keras_params):
        ...
        """
        #        preds = super(AE, self).predict(x,keras_params)
    ...
    def save(self, filename):
        """ Save AE model.
            ...
            AE parameters saved in a .json, while Keras model is stored in .h5 .
        """
        # parameters
        ...
        # echo
        print('Saved AE parameters to {0}.\nKeras model saved to {1}'.format(filename+'.json', filename+'.h5'))


# functions
def load_AE(params_filename, model_filename):
    """ Load stored AE. """
    # load params and create AE
    with open(params_filename) as fd:
        params = json.load(fd)
    autoencoder = AE(**params)

    # load keras model
    autoencoder.ann = load_model(model_filename)
    return autoencoder
